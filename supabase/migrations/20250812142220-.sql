...
CREATE TABLE public.public_booking_requests (
  ...
  -- Add hash for duplicate detection without exposing data
  submission_hash TEXT GENERATED ALWAYS AS (
    encode(digest(email || phone || preferred_date::text, 'sha256'), 'hex')
  ) STORED,
...
-- Create index for performance and duplicate detection
CREATE INDEX idx_public_booking_requests_submission_hash ON public.public_booking_requests(submission_hash);
